classdef EMApexFloat
    ...
    methods
        function obj = EMApexFloat(ID, profiles)
            % ID is the float ID number and profiles is an array of Profile
            % classes
            obj.ID = ID;
            obj.pfls = profiles;
            obj.nPfls = length(profiles);
            
            % Extract some properties such as float time, position and hpid
            % number to hold as single array in this class. 
            obj.hpids = -999*ones(obj.nPfls,1);
            obj.lats = -999*ones(obj.nPfls,1);
            obj.lons = -999*ones(obj.nPfls,1);
            obj.MLT_refs = -999*ones(obj.nPfls,1);
            
            i = 1;
            for pfl = obj.pfls'
                
                obj.hpids(i) = pfl.hpid;
                obj.lats(i) = pfl.lat;
                obj.lons(i) = pfl.lon;
                obj.MLT_refs(i) = pfl.MLT_ref;
                i = i + 1;
                
            end % for
            
            if isunix
                GPS = load(['/noc/users/jc3e13/data/EM-APEX/ema-' ID '-dec/ema-' ID '-gps.mat']);
            else
                GPS = load(['../../data/EM-APEX/ema-' ID '-dec/ema-' ID '-gps.mat']);
            end
            nans = isnan(GPS.lat);
            
            % Calculate interpolated position since individual float
            % positions are suspect. 
            obj.LATS = interp1(GPS.mlt_apf9(~nans), GPS.lat(~nans), obj.MLT_refs);
            obj.LONS = interp1(GPS.mlt_apf9(~nans), GPS.lon(~nans), obj.MLT_refs);

            % Replace any badly interpolated longitudes by recorded ones.
            obj.LATS(isnan(obj.LATS)) = obj.lats(isnan(obj.LATS));
            obj.LONS(isnan(obj.LONS)) = obj.lons(isnan(obj.LONS));
            
            % Calculate distance along track using haversine formula.
            % Calculate northward and eastward depth integrated velocities
            % using this distance assuming that displacement between one
            % half profile and the next is small enough that pythagorean
            % geometry holds to a very good approximation.
            obj.DIST = zeros(size(obj.LATS));
            obj.DIST(2:end) = cumsum(m_lldist(obj.LONS, obj.LATS));
            
            [obj.U, obj.V] = vel_from_track(obj.MLT_refs, obj.LONS, obj.LATS);
            % Quality control.
            obj.U(obj.U > 2) = NaN; obj.V(obj.V > 2) = NaN;
            
            % Also include alternative velocity measurements from the
            % allprof##.mat file because my own calculations are pretty
            % suspect. My zonal velocities seem to be a factor of 10 out
            % while my meridional velocities seem to be a factor of 3 out.
            altData = get_float_data(str2num(ID(1:4)));
            for fn = obj.allProfFNs
                obj.(fn{1}) = altData.(fn{1});
            end
