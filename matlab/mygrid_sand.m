...
% 
% Question over "offset" at line 103 I think I changed this from ilat to (ilat-1)
% But I can't remember now how I figured that out.  Think I discoer it
% when looking at English channel.  Does it still apply for most recent database?
%
...
% (mercator projection)
rad=pi/180;
arg1=log(tan(rad*(45+db_loc(1)/2)));
...
%		offset = ilat*nbytes_per_lat + iwlon(i)*2 ;
		offset = (ilat-1)*nbytes_per_lat + iwlon(i)*2 ;
		status = fseek(fid, offset, 'bof');
% DAS added ,0,'b' because new data is in big endian format
		data(iblat-ilat+1,:)=fread(fid,[1,ielon(i)-iwlon(i)+1],'integer*2',0,'b');
	end
